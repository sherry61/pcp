<template>
      ...
      <div class="content">
        ...
        <div class="actions">
          <button class="action-btn"
                  :class="{ active: activeTab === 'assetUpload' }"
                  @click="setActiveTab('assetUpload'); openAssetUpload()">
            数字资产上传
          </button>
          <button class="action-btn"
                  :class="{ active: activeTab === 'assetDownload' }"
                  @click="setActiveTab('assetDownload'); openAssetDownload()">
            数字资产下载
          </button>
        </div>
  ...
  <div class="contract-actions">
    <div class="contract-btn-row">
      ...
      <button class="contract-btn verify" 
                  ...
                  <td>
                    <div class="upload-section">
                      <input 
                        ...
                        accept=".xlsx,.xls,.csv"
                        :disabled="!asset.contractVerified"
                        class="file-input"
                      />
                      <button 
                        class="upload-btn"
                        @click="startAssetUpload(asset)"
                        :disabled="!asset.contractVerified || !asset.selectedFile || asset.uploading"
                      >
    ...
    for (const buyerAddress of buyerAddresses) {
      try {
        ...
        if (response.data?.success && Array.isArray(response.data.items)) {
          allDownloadableAssets.push(...response.data.items.map(item => ({
            id: item.transaction_id,
            asset_name: item.asset_name || `计算结果_${item.transaction_id}`,
            file_hash: item.result_hash || item.file_hash,
            ...
            compute_status: item.compute_status,
            uploaded_at: item.uploaded_at,
            // 加密计算结果相关字段
            encrypted_result: item.encrypted_result,
            encryption_key: item.encryption_key,
            compute_metadata: item.metadata
          })));
        }
      } catch (error) {
        console.error(`获取地址 ${buyerAddress} 的可下载资产失败:`, error);
      }
    }

    // 去重
    this.downloadList = allDownloadableAssets.filter((asset, index, self) => 
      index === self.findIndex(a => a.id === asset.id)
    );

    // 如果没有真实数据，显示从交易记录获取的数据
